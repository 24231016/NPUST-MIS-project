...
if (empty($_POST["y"])) {
  ...
  if (empty($_POST["x"])) {
   ...
   if (empty($_POST["activetime"])) {
    $activetime="";
    //$active_context = "$active_context is required";
  } else {
    $activetime=test_input($_POST["activetime"]);
    // check if name only contains letters and whitespace
    ...
    $dateline=test_input($_POST["dateline"]);
    // check if name only contains letters and whitespace
...
if ($name!=null&&$x!=null&& $active_title!=null&&$y!=null) {
	# code...
$sql = "INSERT INTO myguests (firstname, active_title,active_context,x,y,activetime,dateline)
VALUES ('$name', '$active_title', '$active_context', '$x','$y','$activetime','$dateline')";
...
<p><input type="hidden" name="y" value="12"></p>
 集合時間:<p> <input type="date" name="activetime"></p>
 參加截止時間:<p> <input type="date" name="dateline"></p>
<br><br><br><br>
